package com.panli.sword.offer._0035;
...
public class Main {
    ...
    public Node copyRandomList_1(Node head) {
        Map<Node, Node> map = new HashMap<Node, Node>(); // 声明HashMap
        Node cur = head;
        // 复制复杂链表节点的值
        while (cur != null) {
            // 复制复杂链表的值
    ...
    public Node copyRandomList_2(Node head) {
        ...
        Node curr = head; // 声明指针指向头节点

        // 复制节点值及next指针，并且每个节点的copy节点都跟在当前节点的后面
        while (curr != null) {
            Node next = curr.next; // 保存下一个next节点
            Node copyNode = new Node(curr.val); // 复制当前节点
        ...
        curr = head;
        while (curr != null) {
            Node next = curr.next.next; // 保存原链表的next节点(curr.next.next是因为curr.next为当前节点的copy，curr.next.next才为真正的下一个节点// )
            Node copyCurr = curr.next; // 取出当前链表的copy节点
        ...
        // 拆分链表
        curr = head; // 原链表的头节点
        Node resNode = curr.next; // copy链表的头节点
        while (curr != null) {
            Node next = curr.next.next; // 保存原链表的下一个节点next，新链表的next的next节点
            Node copyCurr = curr.next; // 拷贝节点
